...
struct gesture_struct ft8006m_gesture_data = {0, 0};

/*****************************************************************************
* Static variables
*****************************************************************************/
static struct fts_gesture_st fts_ft8006m_gesture_data;
...
static ssize_t fts_gesture_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    ft8006m_i2c_read_reg(client, FTS_REG_GESTURE_EN, &val);
    count = sprintf(buf, "Gesture Mode: %s\n", fts_ft8006m_gesture_data.mode ? "On" : "Off");
...
static ssize_t fts_gesture_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (FTS_SYSFS_ECHO_ON(buf))
    {
        ...
        fts_ft8006m_gesture_data.mode = ENABLE;
    }
    else if (FTS_SYSFS_ECHO_OFF(buf))
    {
        FTS_INFO("[GESTURE]disable gesture");
        fts_ft8006m_gesture_data.mode = DISABLE;
    }
...
static ssize_t fts_gesture_buf_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    mutex_lock(&ft8006m_input_dev->mutex);
    count = snprintf(buf, PAGE_SIZE, "Gesture ID: 0x%x\n", fts_ft8006m_gesture_data.header[0]);
    count += snprintf(buf + count, PAGE_SIZE, "Gesture PointNum: %d\n", fts_ft8006m_gesture_data.header[1]);
    count += snprintf(buf + count, PAGE_SIZE, "Gesture Point Buf:\n");
    for (i = 0; i < fts_ft8006m_gesture_data.header[1]; i++)
    {
        count += snprintf(buf + count, PAGE_SIZE, "%3d(%4d,%4d) ", i, fts_ft8006m_gesture_data.coordinate_x[i], fts_ft8006m_gesture_data.coordinate_y[i]);
        if ((i + 1)%4 == 0)
...
/*****************************************************************************
*   Name: ft8006m_create_gesture_sysfs
*  Brief:
*  Input:
* Output:
* Return: 0-success or others-error
*****************************************************************************/
int ft8006m_create_gesture_sysfs(struct i2c_client *client)
{
    int ret = 0;
...
static int fts_gesture_read_buffer(struct i2c_client *client, u8 *buf, int read_bytes)
{
    ...
    {
        ret = ft8006m_i2c_read(client, buf, 1, buf, read_bytes);
    }
    else
    {
        ret = ft8006m_i2c_read(client, buf, 1, buf, I2C_BUFFER_LENGTH_MAXINUM);
        remain_bytes = read_bytes - I2C_BUFFER_LENGTH_MAXINUM;
        for (i = 1; remain_bytes > 0; i++)
        {
            if (remain_bytes <= I2C_BUFFER_LENGTH_MAXINUM)
                ret = ft8006m_i2c_read(client, buf, 0, buf + I2C_BUFFER_LENGTH_MAXINUM * i, remain_bytes);
            else
                ret = ft8006m_i2c_read(client, buf, 0, buf + I2C_BUFFER_LENGTH_MAXINUM * i, I2C_BUFFER_LENGTH_MAXINUM);
...
static int fts_gesture_fw(void)
{
    ...
    switch (ft8006m_chip_types.chip_idh)
    {
        case 0x54:
        case 0x58:
        case 0x64:
...
int ft8006m_gesture_readdata(struct i2c_client *client)
{
    ...
    memset(fts_ft8006m_gesture_data.header, 0, FTS_GESTRUE_POINTS_HEADER);
    memset(fts_ft8006m_gesture_data.coordinate_x, 0, FTS_GESTRUE_POINTS * sizeof(u16));
    memset(fts_ft8006m_gesture_data.coordinate_y, 0, FTS_GESTRUE_POINTS * sizeof(u16));

    buf[0] = FTS_REG_GESTURE_OUTPUT_ADDRESS;
    ret = ft8006m_i2c_read(client, buf, 1, buf, FTS_GESTRUE_POINTS_HEADER);
    if (ret < 0)
    {
    ...
    if (fts_gesture_fw())
    {
        memcpy(fts_ft8006m_gesture_data.header, buf, FTS_GESTRUE_POINTS_HEADER);
        gestrue_id = buf[0];
        pointnum = buf[1];
        ...
        fts_gesture_report(ft8006m_input_dev, gestrue_id);
        for (i = 0; i < pointnum; i++)
        {
            fts_ft8006m_gesture_data.coordinate_x[i] = (((s16) buf[0 + (4 * i + 2)]) & 0x0F) << 8
                                               | (((s16) buf[1 + (4 * i + 2)]) & 0xFF);
            fts_ft8006m_gesture_data.coordinate_y[i] = (((s16) buf[2 + (4 * i + 2)]) & 0x0F) << 8
    ...
    {
        FTS_ERROR("[GESTURE]IC 0x%x need gesture lib to support gestures.", ft8006m_chip_types.chip_idh);
        return 0;
    }
#if 0
    ...
    */
    ...
    for (i = 0; i < pointnum; i++)
    {
        fts_ft8006m_gesture_data.coordinate_x[i] =  (((s16) buf[0 + (4 * i+8)]) & 0x0F) << 8
                                            | (((s16) buf[1 + (4 * i+8)])& 0xFF);
        fts_ft8006m_gesture_data.coordinate_y[i] = (((s16) buf[2 + (4 * i+8)]) & 0x0F) << 8
...
{
    if (fts_ft8006m_gesture_data.mode && fts_ft8006m_gesture_data.active)
    {
        ft8006m_i2c_write_reg(client, 0xD1, 0xff);
        ft8006m_i2c_write_reg(client, 0xD2, 0xff);
        ft8006m_i2c_write_reg(client, 0xD5, 0xff);
        ft8006m_i2c_write_reg(client, 0xD6, 0xff);
        ft8006m_i2c_write_reg(client, 0xD7, 0xff);
        ft8006m_i2c_write_reg(client, 0xD8, 0xff);
        ft8006m_i2c_write_reg(client, FTS_REG_GESTURE_EN, ENABLE);
    }
}

/*****************************************************************************
*   Name: ft8006m_gesture_suspend
*  Brief:
*  Input:
* Output: None
* Return: None
...
int ft8006m_gesture_suspend(struct i2c_client *i2c_client)
{
    ...
    if (fts_ft8006m_gesture_data.mode == 0)
    {
    ...
    for (i = 0; i < 5; i++)
    {
        ft8006m_i2c_write_reg(i2c_client, 0xd1, 0xff);
        ft8006m_i2c_write_reg(i2c_client, 0xd2, 0xff);
        ft8006m_i2c_write_reg(i2c_client, 0xd5, 0xff);
        ft8006m_i2c_write_reg(i2c_client, 0xd6, 0xff);
        ft8006m_i2c_write_reg(i2c_client, 0xd7, 0xff);
        ft8006m_i2c_write_reg(i2c_client, 0xd8, 0xff);
        ft8006m_i2c_write_reg(i2c_client, FTS_REG_GESTURE_EN, 0x01);
        msleep(1);
        ft8006m_i2c_read_reg(i2c_client, FTS_REG_GESTURE_EN, &state);
        if (state == 1)
            break;
    }
    ...
    fts_ft8006m_gesture_data.active = 1;
...
int ft8006m_gesture_resume(struct i2c_client *client)
{
    ...
    if (fts_ft8006m_gesture_data.active == 0)
    {
    ...
    fts_ft8006m_gesture_data.active = 0;
    for (i = 0; i < 5; i++)
    {
        ft8006m_i2c_write_reg(client, FTS_REG_GESTURE_EN, 0x00);
        msleep(1);
        ft8006m_i2c_read_reg(client, FTS_REG_GESTURE_EN, &state);
        if (state == 0)
            break;
    }
...
#else
    ret = kstrtouint(temp, 0, &ft8006m_gesture_data.gesture_all_switch);
    if (ret){
...
int D1_gesture_init(struct input_dev *input_dev)
{
...
int ft8006m_gesture_init(struct input_dev *input_dev, struct i2c_client *client)
{
    ...
    ft8006m_create_gesture_sysfs(client);
#endif
    D1_gesture_init(input_dev);
    fts_ft8006m_gesture_data.mode = 1;
    fts_ft8006m_gesture_data.active = 0;
    FTS_FUNC_EXIT();
    return 0;
}
